...
{
  Serial_println("\r\n----------------------------");
  Serial_println("W25QxFlash的大小 : %d字节",W25Q64_CapacityBuff);
  Serial_println("W25Qx块的大小 : %d字节",W25Q64_BlockBuff);
  Serial_println("W25Qx扇区的大小 : %d字节",W25Q64_SectorBuff);
  Serial_println("W25Qx页的大小 : %d字节",W25Q64_PageBuff);
  Serial_println("W25Qx块数量 : %d",W25Q64_BlockCnt);
  Serial_println("W25Qx扇区数量 : %d",W25Q64_SectorCnt);
  Serial_println("W25Qx页数量 : %d",W25Q64_PageCnt);
  Serial_println("----------------------------\r\n");
}
...
void W25Qx_PageWrite(uint32_t Addr,uint8_t* Data,uint16_t Length)
{
  ...
  HAL_SPI_Transmit(&hspi2, addr_array, sizeof(addr_array), 100);
  HAL_SPI_Transmit(&hspi2, Data, Length, 100);//发送要写的数据
  HAL_GPIO_WritePin(W25Qx_CS_Port, W25Qx_CS_Pin, GPIO_PIN_SET);    // 失能CS
}
...
void W25Qx_SectorWrite(uint32_t Addr,uint8_t* Data,uint32_t Length)
{
  ...
  for (uint32_t i = 0; i < (W25Q64_SectorBuff/W25Q64_PageBuff); i++)//每个扇区16页
  {
